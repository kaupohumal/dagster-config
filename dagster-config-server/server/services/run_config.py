...
def _pipeline_requires_minio(config: dict[str, Any]) -> bool:
    for job in config.get("jobs", []) or []:
        if not isinstance(job, dict):
            continue
        for asset in job.get("assets", []) or []:
            if not isinstance(asset, dict):
                continue
            module_name = asset.get("module")
            if isinstance(module_name, str) and module_name.strip() == "write_to_csv":
                return True
    return False


def _resolve_minio_resource(config: dict[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    resource_name = resource.get("name") if isinstance(resource, dict) else None
    if not isinstance(resource_name, str) or not resource_name.strip():
        raise LookupError("MinIO resource must define a non-empty 'name'.")
...
def apply_minio_resource_config(
    ...
    if not _pipeline_requires_minio(pipeline_config):
        return merged_config

    resource_name, yaml_resource_params = _resolve_minio_resource(pipeline_config)
